/**
 * dsh-market, browser half — dynamic-plugin source form.
 *
 * 当前运行形态：dsh Web UI 的动态 Cordis 插件（会话级、临时）。
 * 本文件为动态插件形态源码，apply 内直接使用动态运行时内建
 * `styles` / `host` / `React`。持久化 npm bundle 适配
 * （ModuleLoader factory 形式 + @deepseek-ai/dsh-client-runtime）
 * 进行中，见仓库 Roadmap。
 *
 * UI：注册官方 `settings.plugins.tab` 扩展点（Plugins 设置区「发现插件」），
 * 渲染插件卡片流；全部数据经 host.call 从 host 面获取，客户端不直连网络。
 */
...
.mkt-tabrow { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
.mkt-sortgroup { display: inline-flex; gap: 4px; flex-wrap: wrap; align-items: center; }
.mkt-sortlabel { color: var(--dsw-alias-label-secondary); font-size: 12px; }
.mkt-controls { display: flex; align-items: center; gap: 6px; flex: 1; min-width: 0; flex-wrap: nowrap; }
.mkt-tab { background: transparent; border: 1px solid var(--dsw-alias-border-l1); color: var(--dsw-alias-label-secondary); border-radius: 6px; padding: 4px 10px; font-size: 12px; cursor: pointer; white-space: nowrap; }
...
.mkt-input { flex: 1; min-width: 0; background: var(--dsw-alias-bg-layer-1); border: 1px solid var(--dsw-alias-border-l1); color: var(--dsw-alias-label-primary); border-radius: 6px; padding: 6px 8px; font-size: 12px; }
.mkt-input:focus { border-color: var(--dsw-alias-brand-primary); outline: none; }
.mkt-select { background: var(--dsw-alias-bg-layer-1); border: 1px solid var(--dsw-alias-border-l1); color: var(--dsw-alias-label-primary); border-radius: 6px; padding: 5px 8px; font-size: 12px; max-width: 120px; cursor: pointer; }
...
.mkt-delta-pos { color: var(--dsw-alias-state-success-primary, #22c55e); }
.mkt-delta-neg { color: var(--dsw-alias-state-error-primary, #ef4444); }
.mkt-delta-zero { color: var(--dsw-alias-label-secondary); }
...
export default {
  apply(ctx) {
    ...
    function MarketPanel() {
      ...
      const [data, setData] = React.useState({ repos: [], total: 0, pluginTotal: 0, curatedCount: 0, fetchedAt: '', languages: [], deltaDays: null })
      ...
      const [liveQ] = React.useState(() => {
        if (!timerSvc || !timerSvc.debounce) return null
        return timerSvc.debounce((v) => {
          setParams((p) => Object.assign({}, p, { q: v, page: 1 }))
        }, 400)
      })
      ...
      React.useEffect(() => {
        ...
        setLoading(true)
        setError(null)
        host.call('market.search', params).then((res) => {
          if (cancelled) return
          if (res && res.ok) {
            setData({
              repos: res.repos,
      ...
      function refresh() {
        setLoading(true)
        setError(null)
        host.call('market.refresh').then((res) => {
          if (res && res.ok) {
            setTick((t) => t + 1)
          } else {
      ...
      function onQInput(ev) {
        ...
        setQInput(v)
        if (liveQ) { liveQ(v) } else { commitQ(v) }
      }
      ...
      function beginInstall(repo) {
        ...
        host.call('market.install', { spec, kind: 'github' }).then((res) => {
      ...
      function confirmInstall() {
        ...
        host.call('market.install', { spec: c.spec, kind: 'github', confirmed: true }).then((res) => {
      ...
      function repoCard(repo) {
        ...
        rows.push(el('div', { className: 'mkt-card', key: repo.full_name },
          ...
          el('div', { className: 'mkt-meta' },
            (function () {
              const parts = [repo.language || '未知语言']
              if (repo.stars_delta !== undefined) {
                const sign = repo.stars_delta >= 0 ? '+' : ''
                const cls = repo.stars_delta > 0 ? 'mkt-delta-pos' : repo.stars_delta < 0 ? 'mkt-delta-neg' : 'mkt-delta-zero'
                parts.push('★ ' + repo.stars + ' (' + el('span', { className: cls }, sign + repo.stars_delta) + ')')
              } else {
                parts.push('★ ' + repo.stars)
              }
              parts.push('fork ' + repo.forks)
              parts.push('更新 ' + (repo.pushed_at ? repo.pushed_at.slice(0, 10) : '?'))
              return parts.join(' · ')
            })(),
          ),
      ...
      const sortTabs = [
        ['stars', '总星数'],
        ['stars_delta', data.deltaDays ? '近期飙升' : '飙升'],
        ['created', '最新发布'],
        ['pushed', '最近推送'],
      ]
      ...
      return el('div', { className: 'mkt-panel' },
        ...
        el('div', { className: 'mkt-tabrow' },
          ...
          el('div', { className: 'mkt-controls' },
            el('input', {
              className: 'mkt-input',
              placeholder: '搜索仓库名/描述',
              value: qInput,
              onChange: onQInput,
              onKeyDown: (ev) => { if (ev.key === 'Enter') commitQ(qInput.trim()) },
            }),
            el('select', {
            ...
            el('button', { className: 'mkt-btn ghost', onClick: refresh, title: '刷新数据' }, '↻'),
          ),
        ),
        filtered
        ...
        loading
          ? el('div', { className: 'mkt-sub' }, '正在从 GitHub 拉取全量插件数据（约 30 秒，仅首次）…')
          : data.repos.length === 0
