...
export function apply(ctx) {
  ...
  async function enrich(repos) {
    ...
    console.log('[market] enriched: curated ' + curatedCount)
  }
  ...
  async function loadDeltas() {
    ...
    deltaPromise = (async () => {
      try {
        ...
        const got = await fetchRaw(apiUrl, { 'User-Agent': 'dsh-plugins-market', 'Accept': 'application/vnd.github+json' }, 100000)
        if (got.status !== 200 || got.text === null) return null
        const dirListing = JSON.parse(got.text)
  ...
  async function handle(req, res) {
    ...
    if (pathname === '/market-api/search' && req.method === 'GET') {
      ...
      const sort = sortParam === 'created' || sortParam === 'pushed' || sortParam === 'stars_delta' ? sortParam : 'stars'
